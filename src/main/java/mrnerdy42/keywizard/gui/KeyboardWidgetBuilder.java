...
public class KeyboardWidgetBuilder {
	...
	public static KeyboardWidget StandardKeyboard(KeyWizardScreen keyWizardScreen, int anchorX, int anchorY, int width, int height) {
		...
		currentX = addHorizontalRow(kb, GLFW.GLFW_KEY_F1, GLFW.GLFW_KEY_F12, 0, currentY, keyWidth, keyHeight, keySpacing);
		...
		currentY += keyHeight + keySpacing;
		kb.addKey(0, currentY, keyWidth*2+keySpacing, keyHeight, keySpacing, GLFW.GLFW_KEY_LEFT_SHIFT);
		currentX = keyWidth*2+keySpacing*2;
		...
		kb.addKey(currentX, currentY, (keyWidth*3+keySpacing*2)+nudge, keyHeight, keySpacing, GLFW.GLFW_KEY_RIGHT_SHIFT);
